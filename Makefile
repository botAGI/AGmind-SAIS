...
test-core-detector-pin-image:
	docker build --pull=false --platform linux/arm64 --build-arg PYTHON_IMAGE="$(PYTHON_IMAGE)" --tag "$(CORE_DETECTOR_PIN_IMAGE)" .
	docker run --rm --platform linux/arm64 --network none --read-only --mount "type=bind,src=$(PWD)/deploy/falco/rules.d/agmind-pcc.yaml,dst=/expected/agmind-pcc.yaml,readonly" -e PYTHONPATH=/app/core "$(CORE_DETECTOR_PIN_IMAGE)" python -c 'import hashlib, os, platform, pwd, runpy, sqlite3, sys; from pathlib import Path; from agmind_immune.canonicaljson import pcc_detector_bundle_sha256; from agmind_immune.correlation.authority import _load_pinned_detector_bundle; from agmind_immune.correlation.primitives import load_pinned_special_use_registry, special_use_registry_is_issued; assert pwd.getpwuid(os.geteuid()).pw_name == "sais"; actual = _load_pinned_detector_bundle(); expected = pcc_detector_bundle_sha256(Path("/expected/agmind-pcc.yaml").read_bytes()); assert actual == expected, (actual, expected); schema = Path("/app/core/agmind_immune/evidence/schema.sql"); assert hashlib.sha256(schema.read_bytes()).hexdigest() == "d4a5d563ca3964cbe4ed276882a4b4def95fb756fc67a6777fddf5de38b1619d"; registry = load_pinned_special_use_registry(Path("/usr/share/agmind-sais/ipv4-special-use.csv")); assert special_use_registry_is_issued(registry); runpy.run_path("/app/core/tests/correlation/sqlite_integrity_runtime_smoke.py", run_name="__main__"); print(f"platform=linux/{platform.machine()} python={sys.version.split()[0]} sqlite={sqlite3.sqlite_version} detector={actual} registry={registry.authority_sha256}")'
