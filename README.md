...
Hunter — локальная обесцензуренная модель на железе оператора; она
используется только как недоверенный read-only слой — для гипотез и
объяснений поверх уже зафиксированных фактов.
